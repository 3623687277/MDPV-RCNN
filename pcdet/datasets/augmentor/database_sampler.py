...
class DataBaseSampler(object):
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                obj_points = np.fromfile(str(file_path), dtype=np.float32).reshape(
                    [-1, self.sampler_cfg.NUM_POINT_FEATURES-3])
                if obj_points.shape[0] != info['num_points_in_gt']:
                    obj_points = np.fromfile(str(file_path), dtype=np.float64).reshape(-1, self.sampler_cfg.NUM_POINT_FEATURES-3)
            # print(obj_points.shape,info['num_points_in_gt'],'obj_points')
        ...
        # print(obj_points.shape, points.shape,'obj_points的点数')#不带颜色 4,4    带颜色   4,7
        if self.sampler_cfg.get('FILTER_OBJ_POINTS_BY_TIMESTAMP', False) or obj_points.shape[-1]+3 != points.shape[-1]:
        ...
        # print(points.shape,'large_sampled_gt_boxes')
        points = box_utils.remove_points_in_boxes3d(points, large_sampled_gt_boxes)
        # print(points.shape,'pointsdianshu')
        # print(obj_points.shape,'objectdianshu')
        # obj_points四维增加三维
        array_3d = np.zeros((obj_points.shape[0], 3))
        # print(array_3d.shape,'array_3d')
        obj_points = np.concatenate([obj_points, array_3d], axis=1)
        # print(obj_points)
        ...
        # print(data_dict['points'].shape,'data_dict加点')
