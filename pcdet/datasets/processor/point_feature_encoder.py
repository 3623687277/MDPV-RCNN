...
class PointFeatureEncoder(object):
    ...
    def absolute_coordinates_encoding(self, points=None):
        ...
        image_path = '/media/ubuntu/F/lp/OpenPCDet+deformable原版/data/kitti/training/image_2/000031.png'
        calib_path = '/media/ubuntu/F/lp/OpenPCDet+deformable原版/data/kitti/training/calib/000031.txt'
        point_in_lidar = points[:,:3]
        ...
        points = np.hstack((valid_points, colors))  # N x 6
        # print(colored_point.shape)
        #
        # points = colored_point[:, :3]
        # colors = colored_point[:, 3:]/255
        ...
        # 保存带颜色的点云图
        # np.save('path_to_save colored_points.npy', colored_point)
        # 获取深度图
        ...
        # print(points.shape[-1],len(self.src_feature_list),'len(self.src_feature_list)')

        assert points.shape[-1] == len(self.src_feature_list)-1
        # points1=points[:, 0:8]
